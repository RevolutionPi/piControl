...
#define MG_AL_TIMEOUT            (MG_AL_MAX_RETRY-1)
		...
		if (piCore_g.abMGateActive[inst]) {
			...
			while (!bDone) {
				...
				if (pPacket_l) {
					...
					default:
						// ignore packet
						pr_err("%d: MG_LL_recv: returned unknown packet 0x%x\n", spi_selected_chip(),
...
#endif
					if (!bSend && kbUT_TimerExpired(&AL_Data_s[inst].AL_Timeout)) {
						pr_info_modgate("%d: no data timeout %d\n", spi_selected_chip(), retry);
						if (retry < 5) {
							bSend = bTRUE;
							retry++;
						} else {

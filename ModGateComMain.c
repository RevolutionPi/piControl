...
{
	MODGATECOM_Packet *pPacket_l =
	    (MODGATECOM_Packet *) kbUT_malloc(LL_pvPacketHeapHandle_g, 3,
					      MODGATE_TL_HEADER_LEN + sizeof(MODGATECOM_IDResp));
	if (pPacket_l == 0) {
...
INT32U MODGATECOM_send_cyclicPD(ALHandle alHdl)
{
	...
	MODGATECOM_Packet *pPacket_l =
	    (MODGATECOM_Packet *) kbUT_malloc(LL_pvPacketHeapHandle_g, 4,
					      MODGATE_TL_HEADER_LEN + sizeof(MODGATECOM_CyclicPD) +

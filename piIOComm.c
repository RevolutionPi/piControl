    ...
    INT8U *pi8uRecvData_p, INT8U i8uRecvDataLen_p)
{
    ...
    {
#ifdef DEBUG_SERIALCOMM
        DF_PRINTK("send gateprotocol addr %d cmd 0x%04x\n", suSendTelegram_l.i8uDstAdr, suSendTelegram_l.i16uCmd);
#endif
        ...
        {
            if (suRecvTelegram_l.ai8uData[suRecvTelegram_l.i8uDataLen] != piIoComm_Crc8((INT8U*)&suRecvTelegram_l, RS485_HDRLEN + suRecvTelegram_l.i8uDataLen))
            {
#ifdef DEBUG_SERIALCOMM
                DF_PRINTK("recv gateprotocol crc error: len=%d, %02x %02x %02x %02x %02x %02x %02x %02x\n",
                          suRecvTelegram_l.i8uDataLen,
                          suRecvTelegram_l.ai8uData[0], suRecvTelegram_l.ai8uData[1], suRecvTelegram_l.ai8uData[2], suRecvTelegram_l.ai8uData[3],
                          suRecvTelegram_l.ai8uData[4], suRecvTelegram_l.ai8uData[5], suRecvTelegram_l.ai8uData[6], suRecvTelegram_l.ai8uData[7]);
#endif
                i32uRv_l = 4;
            }
            ...
            {
#ifdef DEBUG_SERIALCOMM
                DF_PRINTK("recv gateprotocol addr %d cmd 0x%04x\n", suRecvTelegram_l.i8uSrcAdr, suRecvTelegram_l.i16uCmd);
#endif
...
INT32S piIoComm_sendCyclicTelegram0(INT8U i8uDevice_p)
{
    ...
    len_l = RevPiScan.dev[i8uDevice_p].sId.i16uFBS_OutputLength;

    sRequest_l.uHeader.sHeaderTyp1.bitAddress = RevPiScan.dev[i8uDevice_p].i8uAddress;
    ...
    sRequest_l.uHeader.sHeaderTyp1.bitCommand = 0;

    rt_mutex_lock(&piDev_g.lockPI);
    memcpy(sRequest_l.ai8uData, piDev_g.ai8uPI + RevPiScan.dev[i8uDevice_p].i16uOutputOffset, len_l);
    rt_mutex_unlock(&piDev_g.lockPI);

    sRequest_l.ai8uData[len_l] = piIoComm_Crc8((INT8U*)&sRequest_l, IOPROTOCOL_HEADER_LENGTH + len_l);

#ifdef DEBUG_DEVICE_IO
    if (last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] != sRequest_l.ai8uData[0] || last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] != sRequest_l.ai8uData[1])
    {
        last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] = sRequest_l.ai8uData[0];
        last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] = sRequest_l.ai8uData[1];
        DF_PRINTK("dev %2d: send cyclic Data addr %d + %d output 0x%02x 0x%02x\n",
                  RevPiDevice.dev[i8uDevice_p].i8uAddress,
                  sRequest_l.uHeader.sHeaderTyp1.bitAddress, RevPiDevice.dev[i8uDevice_p].i16uOutputOffset,
    ...
    if (ret == 0)
    {
        len_l = RevPiScan.dev[i8uDevice_p].sId.i16uFBS_InputLength;
        ...
        if (ret > 0)
        {
            ...
            {
                rt_mutex_lock(&piDev_g.lockPI);
                memcpy(piDev_g.ai8uPI + RevPiScan.dev[i8uDevice_p].i16uInputOffset, sResponse_l.ai8uData, len_l);
                rt_mutex_unlock(&piDev_g.lockPI);

#ifdef DEBUG_DEVICE_IO
                if (last_in[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] != sResponse_l.ai8uData[0] || last_in[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] != sResponse_l.ai8uData[1])
                {
                    last_in[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] = sResponse_l.ai8uData[0];
                    last_in[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] = sResponse_l.ai8uData[1];
                    DF_PRINTK("dev %2d: recv cyclic Data addr %d + %d input 0x%02x 0x%02x\n\n",
                              RevPiDevice.dev[i8uDevice_p].i8uAddress,
                              sResponse_l.uHeader.sHeaderTyp1.bitAddress, RevPiDevice.dev[i8uDevice_p].i16uInputOffset,
        ...
        else
        {
            i32uRv_l = 2;
#ifdef DEBUG_DEVICE_IO
            DF_PRINTK("dev %2d: recv ioprotocol timeout error\n",
                      RevPiDevice.dev[i8uDevice_p].i8uAddress
                      );
#endif
        }
...
INT32S piIoComm_gotoGateProtocol(void)
{
    ...
    if (last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] != sRequest_l.ai8uData[0] || last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] != sRequest_l.ai8uData[1])
    {
        last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][0] = sRequest_l.ai8uData[0];
        last_out[RevPiDevice.dev[i8uDevice_p].i8uAddress][1] = sRequest_l.ai8uData[1];
        DF_PRINTK("dev all: send gotoGateProtocol\n");
    }
#endif
